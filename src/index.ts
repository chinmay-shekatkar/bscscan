...
async function getData() {
  try {
    ...
    const response = await axios.get(`https://api.bscscan.com/api?module=account&action=tokentx&address=${process.argv.slice(2)}&startblock=0&endblock=999999999&sort=asc&apikey=${apiKey}`);
    console.log();
    console.log("Token Transactions");
    response.data.result.forEach(function (element: any) {
      console.log("------------------------------------------------------------------------------");
    ...
    for (let key in map) {
      ...
      console.log("----------------------------------------");
      const response = await axios.get(`https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress=${map[key]}&address=${process.argv.slice(2)}&tag=latest&apikey=${apiKey}`);
